...
module.exports = async function (channel, message, options) {
  const init = new Promise(async (resolve) => {
    ...
    // Assign Webhook
    let hook = webhooks.find((w) => w.name === "http://bot.crying.tk");
    if (!hook) {
      try {
        hook = await channel.createWebhook("http://bot.crying.tk", {
          avatar: `http://bot.crying.tk/logo.png`,
        });
      } catch (e) {
        hook = await channel.createWebhook(
          "http://bot.crying.tk",
          `http://bot.crying.tk/logo.png`
        );
      }
