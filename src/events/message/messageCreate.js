    ...
    try {
      ...
      if (message.content.match(mentionRegex)) {
        ...
        const embed = new MessageEmbed()
          .setTitle("Hello, I'm Crying. What's Up?")
          .addField(`Prefix`, proofita, true)
          .addField(`Usage`, proofitaa, true)
          .setDescription(
            `\nIf you like Crying, Consider [voting](https://top.gg/bot/767705905235099658), or [inviting](${config.invite_link}) it to your server! Thank you for using Crying, we hope you enjoy it, as we always look forward to improve the bot`
          )
          .setFooter("Thank you for using Crying!!")
          .setColor("#FF2C98");
      ...
      if (!this.client.config.developers.includes(message.author.id)) {
        if (maintenance && maintenance.toggle == "true") {
          ...
          message.channel.sendCustom(
            `Crying is currently undergoing maintenance which won't allow anyone to access Crying's Commands. Feel free to try again later. For updates: ${config.discord}`
          );
          ...
          return message.channel
            .sendCustom(
              ` ${
                ...
                number === 1
                  ? "*Did You know that Crying has its own dashboard? `http://bot.crying.tk/dashboard`*"
                  : ""
              }${
                number === 2
                  ? "*You can check our top.gg page at `https://vote.crying.tk`*"
                  : ""
              }`
            )
            .then((s) => {
